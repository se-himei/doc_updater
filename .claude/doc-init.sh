...
- ゴールデンテスト画像の確認:
  - `find test -name "*.png"` でゴールデンテスト画像の存在を確認
  - 新しい画面が追加された場合は、対応するゴールデンテスト画像があるか確認
...
- 画面に関する更新があれば記述
- ゴールデンテスト画像の掲載:
  - 各画面セクション（S001, S002等）の「概要」の後に「画面イメージ（ゴールデンテスト）」セクションを追加
  - test/goldens/ および test/screens/goldens/ ディレクトリにある画像を相対パスで参照
  - 画像の説明（データなし状態、データあり状態など）を各画像に付ける
  - 例: `![メイン画面（空）](../../test/goldens/main_screen_empty.png)`
...
- 日付・バージョン情報を更新
- ゴールデンテスト画像の確認:
  - すべての画面イメージが正しく参照されているか確認
  - 画像パスが相対パスで正しく記載されているか確認
  - 画像の説明が適切か確認（データなし、データあり等）
  - 新しい画面が追加された場合、対応する画像がドキュメントに掲載されているか確認
